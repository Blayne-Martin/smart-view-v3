...
const MetricChart: React.FC<MetricChartProps> = ({ data, dataKey, label, unit, color }) => {
  ...
  const formatted = data.map((d) => ({
    ...d,
    _ts: d.recorded_at.substring(5, 16),
  }))
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-5">
      ...
      <div role="img" aria-label={`${label} over time. Average ${avg}${unit}, min ${min}${unit}, max ${max}${unit}`}>
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={formatted} margin={{ top: 4, right: 8, left: 0, bottom: 4 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
            <XAxis dataKey="_ts" tick={{ fontSize: 10, fill: '#9ca3af' }} interval="preserveStartEnd" tickLine={false} />
            ...
            />
            <Line type="monotone" dataKey={dataKey as string} stroke={color} dot={false} strokeWidth={2} isAnimationActive={false} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
...
export const DeviceDetail: React.FC = () => {
  const params = useParams<{ customerId: string; deviceId: string }>()
  const customerId = params?.customerId ?? ''
  const deviceId = params?.deviceId ?? ''
  const [days, setDays] = useState<TimeRange>(7)

  const { data: device, isLoading: deviceLoading, isError: deviceError } = useDevice(customerId, deviceId)
  const { data: history, isLoading: historyLoading } = useDeviceHistory(customerId, deviceId, days)
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        <div className="bg-white border border-gray-200 rounded-2xl p-6">
          <nav aria-label="Breadcrumb" className="text-sm text-gray-500 mb-2">
            <Link href={`/customers/${customerId}`} className="hover:underline text-blue-600">Customer</Link>
            {' / '}
        ...
        <section aria-label="Historical data">
          ...
          {history && history.data.length > 0 && (
            <div className="space-y-4">
              <MetricChart data={history.data} dataKey="download_mbps" label="Download" unit=" Mbps" color="#3b82f6" />
              <MetricChart data={history.data} dataKey="upload_mbps"   label="Upload"   unit=" Mbps" color="#10b981" />
              <MetricChart data={history.data} dataKey="latency_ms"    label="Latency"  unit=" ms"   color="#f59e0b" />
              {isWifi && (
                <MetricChart data={history.data} dataKey="rssi_dbm" label="RSSI (Signal Strength)" unit=" dBm" color="#8b5cf6" />
              )}
              {history.samplingApplied && (
                <p className="text-xs text-gray-400 text-right">Data was downsampled for visualisation.</p>
              )}
            </div>
          )}
