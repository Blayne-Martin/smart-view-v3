...
export const modemAPI = {
  ...
  getHistory: async (
    ...
    days: 1 | 7 | 30 | 90 = 7,
    limit = 200
  ): Promise<ModemHistoryResponse> => {
    const response = await apiClient.get(
      `/modems/${customerId}/history`,
      { params: { days, limit } }
    ) as any
    return {
      data: response.data as ModemStat[],
      samplingApplied: response.metadata?.samplingApplied ?? false,
    }
  },

  streamStats: (customerId: string) => {
    return new EventSource(`${API_BASE}/stream/modems/${customerId}`)
  },
}
...
export type DeviceType =
  ...
  | 'games_console' | 'smart_tv' | 'wifi_extender'
  | 'router' | 'iot' | 'other'
...
export const deviceAPI = {
  getAll: async (
    ...
    limit = 50,
    offset = 0
  ): Promise<{ data: DeviceWithStats[]; pagination: { total: number; limit: number; offset: number; hasMore: boolean } }> => {
    const response = await apiClient.get(`/customers/${customerId}/devices`, {
      params: { limit, offset },
    }) as any
    return {
      data: response.data as DeviceWithStats[],
      pagination: response.metadata?.pagination ?? { total: 0, limit, offset, hasMore: false },
    }
  },

  getTopology: async (customerId: string): Promise<DeviceWithStats[]> => {
    const response = await apiClient.get<DeviceWithStats[]>(
      `/customers/${customerId}/devices/topology`
    )
    return response.data
  },
  ...
  getHistory: async (
    ...
    days: 1 | 7 | 30 | 90 = 7,
    limit = 200
  ): Promise<{ data: DeviceHistoryRecord[]; samplingApplied: boolean }> => {
    const response = await apiClient.get(
      `/customers/${customerId}/devices/${deviceId}/history`,
      { params: { days, limit } }
    ) as any
    return {
      data: response.data as DeviceHistoryRecord[],
      samplingApplied: response.metadata?.samplingApplied ?? false,
    }
  },
}
...
export const fleetAPI = {
  getWorstPerformers: async (limit = 10, status?: 'Good' | 'Warn' | 'Bad') => {
    const response = await apiClient.get<ModemStat[]>('/fleet/worst-performers', {
      params: { limit, status },
    })
    return response.data
  },
  ...
  getNetworkHistory: async (days: 7 | 30 | 90 = 30) => {
    const response = await apiClient.get<{
      days: number
      summary: { avg_latency: number; avg_packet_loss: number; avg_jitter: number; avg_snr: number }
      daily: {
        date: string
        avg_latency: number; min_latency: number; max_latency: number
        avg_packet_loss: number; min_packet_loss: number; max_packet_loss: number
        avg_jitter: number; min_jitter: number; max_jitter: number
        avg_snr: number; min_snr: number; max_snr: number
        pct_good: number; pct_warn: number; pct_bad: number
        sample_count: number
      }[]
      distribution: { total: number; latency: number[]; packet_loss: number[]; jitter: number[]; snr: number[] }
    }>('/fleet/network-history', { params: { days } })
    return response.data
  },
