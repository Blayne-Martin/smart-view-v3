...
/**
 * Worst Performers Table Component
 */
...
const WorstPerformersTable: React.FC<WorstPerformersTableProps> = ({
  data,
  isLoading,
  onRowClick,
}) => {
  ...
  return (
    <div className="overflow-x-auto">
      <table
        role="grid"
        aria-label="Worst performing modems"
      ...
      >
        <thead className="bg-gray-50 border-b border-gray-200">
          <tr>
            ...
            <th
              scope="col"
              className="px-6 py-3 text-right font-semibold text-gray-900"
            >
              Latency (ms)
            </th>
            <th
              ...
              <td className="px-6 py-4">
                <span
                  className={`
                    ...
                    ${getStatusBadge(modem.health_score)}
                  `}
                >
                  {modem.health_score === 'Good'
                    ? 'Healthy'
                    : modem.health_score === 'Warn'
                      ? 'Warning'
                      : 'Critical'}
                </span>
              </td>
              <td className="px-6 py-4 text-right text-gray-600">
                {modem.latency.toFixed(2)}
              </td>
              <td className="px-6 py-4 text-right text-gray-600">
                {modem.packet_loss.toFixed(2)}
              </td>
              <td className="px-6 py-4 text-right text-gray-600">
                {modem.jitter.toFixed(2)}
              </td>
              <td className="px-6 py-4 text-right text-gray-600">
                {modem.snr.toFixed(2)}
              </td>
...
/**
 * Fleet Dashboard Home Page
 * 
 * Shows:
 * - Summary tiles (Total, Healthy, Warning, Critical)
 * - Worst performers table with drill-down to customer detail
...
export const FleetDashboard: React.FC = () => {
  ...
  const { data: worstPerformers, isLoading: performersLoading } =
    useWorstPerformers(selectedStatus === 'All' ? undefined : selectedStatus === 'Good' ? 'Good' : selectedStatus === 'Warn' ? 'Warn' : 'Bad')
          ...
          <HealthStatusFilter
            ...
            counts={
              summary
                ? {
                    all: summary.total,
                    good: summary.healthy,
                    warn: summary.warning,
                    bad: summary.critical,
                  }
                : undefined
            }
          />
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow">
          <div className="px-6 py-4 border-b border-gray-200">
            ...
            <p className="text-sm text-gray-600 mt-1">
              Top 10 modems requiring attention
            </p>
          </div>
