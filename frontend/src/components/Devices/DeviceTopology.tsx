...
const H_STEP      = NODE_WIDTH + H_GAP   // 180px
const V_STEP      = NODE_HEIGHT + V_GAP  // 160px
const PAD         = 40   // canvas padding
const CANVAS_EXTRA = 200 // extra padding so nodes have room to move
...
/**
 * Recursively compute the subtree width (in columns) needed for a node.
 */
...
/**
 * Recursively assign x/y positions. `offsetX` is the left boundary of the
 * space allocated to this subtree (in pixels).
 */
function assignPositions(
  ...
  const subtreeWidth = leafCount * H_STEP - H_GAP  // total px width
...
function computeLayout(roots: DeviceNode[]): LayoutResult {
  ...
  return {
    nodes,
    edges,
    totalWidth: maxX + PAD + CANVAS_EXTRA,
    totalHeight: maxY + PAD + CANVAS_EXTRA,
  }
}
...
const SvgNode: React.FC<SvgNodeProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Header background (top-rounded only) */}
      <rect
        width={NODE_WIDTH}
        height={HEADER_H + R}
        fill={headerBg}
      ...
      {/* Icon (foreignObject for emoji rendering) */}
...
const Legend: React.FC = () => {
  ...
  return (
    <div
      className="absolute top-4 right-4 bg-white border border-gray-200 rounded-xl shadow-sm p-3 text-xs"
      aria-label="Edge style legend"
    >
...
export const DeviceTopology: React.FC = () => {
  ...
  // Node positions state — initialised from layout, updated on drag
  const [nodePositions, setNodePositions] = useState<NodePositions>(new Map())

  // Sync positions whenever layout changes (new data load or roots change)
  useEffect(() => {
    if (!layout) return
  ...
  // Drag state stored in a ref so it does not cause re-renders during drag
  const dragState = useRef<DragState | null>(null)
  const svgRef = useRef<SVGSVGElement | null>(null)

  // Canvas bounds derived from layout
  ...
  const moveDrag = useCallback((clientX: number, clientY: number) => {
    ...
    // Mark as moved if pointer travelled more than 4px
    ...
    // Clamp so node cannot go off the top/left edge; right/bottom free (canvas has extra room)
    const clampedX = Math.max(0, rawX)
    const clampedY = Math.max(0, rawY)

    setNodePositions(prev => {
      const next = new Map(prev)
      next.set(nodeId, { x: clampedX, y: clampedY })
      return next
    })
  }, [])
  ...
  const endDrag = useCallback((clientX?: number, clientY?: number) => {
    ...
    // If no movement detected treat as a click → navigate
    if (!moved && customerId) {
      navigate(`/customers/${customerId}/devices/${nodeId}`)
    }
  }, [customerId, navigate])

  // ── Mouse event handlers (attached to SVG) ──────────────────────────────────
  ...
  const handleSvgMouseLeave = useCallback(() => {
    // Cancel drag without triggering navigation if the cursor leaves the SVG
    dragState.current = null
  }, [])

  // ── Touch event handlers (attached to SVG) ──────────────────────────────────
  ...
  // ── Per-node mouse/touch down handlers (passed to SvgNode) ──────────────────
        ...
        <div className="mb-6 bg-white border border-gray-200 rounded-2xl p-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <Link
                to={`/customers/${customerId}/devices`}
          ...
          <p className="text-sm text-gray-500 mt-1">
            Click any device node to view its detail. Drag nodes to reposition them.
          </p>
        </div>
        ...
        {!isLoading && layout && nodePositions.size > 0 && (
          ...
          >
            ...
            >
              ...
              {/* Edge coordinates are derived from nodePositions on every render */}
              <g aria-hidden="true">
