...
// ── Health Status Trend (stacked area) ────────────────────────────────────────
...
// ── Single metric trend with min/max band ────────────────────────────────────
    ...
    <SectionCard title={title} subtitle={`Daily average with min/max range · ${unit}`}>
      <ResponsiveContainer width="100%" height={200}>
        <ComposedChart data={chartData} margin={{ top: 4, right: 8, left: -8, bottom: 0 }}>
          ...
          {/* Band area using two invisible lines + area between them */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {isLoading ? <LoadingSkeleton /> : data && (
          <div className="space-y-6">
            ...
            {/* Health status stacked area */}
            <HealthTrendChart data={data.daily} days={days} />

            {/* 4 metric trend charts */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <MetricTrendChart
