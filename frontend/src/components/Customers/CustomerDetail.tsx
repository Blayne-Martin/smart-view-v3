...
/**
 * Gauge Component for displaying modem stats
 */
    ...
    <div className="bg-white border border-gray-200 rounded-2xl p-6">
      ...
      >
        ...
        <p className="text-xs text-gray-600 text-center">
          {isWarning
            ? `⚠ Above threshold (${warningThreshold}${unit})`
            : `✓ Normal`}
        </p>
      </div>
    </div>
...
export const CustomerDetail: React.FC = () => {
  ...
  const { data: currentStats, isLoading: statsLoading, isStreaming } =
    useModemStats(customerId || '', true)
  const { data: history, isLoading: historyLoading } = useModemHistory(
    customerId || '',
    timeRange
  )
  ...
  const healthColor = currentStats
    ? getHealthColor(currentStats.health_score)
    : '#999'
            ...
            <div className="flex items-center justify-between">
              ...
              {currentStats && (
                <div className="text-center">
                  ...
                  >
                    {currentStats.health_score === 'Good'
                      ? '✓'
                      : currentStats.health_score === 'Warn'
                        ? '⚠'
                        : '✕'}
                  </div>
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <Gauge
                ...
                ariaLabel={`Latency: ${currentStats.latency} milliseconds`}
              />
              <Gauge
                label="Jitter"
                value={currentStats.jitter}
                unit="ms"
                min={0}
                max={50}
                warningThreshold={20}
                ariaLabel={`Jitter: ${currentStats.jitter} milliseconds`}
              />
              <Gauge
                label="Packet Loss"
                value={currentStats.packet_loss}
                unit="%"
                min={0}
                max={10}
                warningThreshold={5}
                ariaLabel={`Packet Loss: ${currentStats.packet_loss}%`}
              />
              <Gauge
                label="SNR"
                value={currentStats.snr}
                unit="dB"
                min={0}
                max={50}
                warningThreshold={20}
                ariaLabel={`Signal-to-Noise Ratio: ${currentStats.snr} dB`}
              />
            </div>
          </div>
        )}
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-6">
              ...
              {history.samplingApplied && (
                <p className="text-xs text-gray-500 mt-4">
                  ℹ Data was downsampled from {history.originalCount} points to
                  {history.dataPoints} for better visualization
                </p>
              )}
            </div>
